...
export function MarketTrends({ products }: MarketTrendsProps) {
  ...
  const formattedDate = language === 'bn' 
    ? `${today.getDate()} ${format(today, 'MMMM', { locale: bn })}, ${today.getFullYear()}`
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-semibold">
            {language === 'bn' ? 'আজকের বাজার' : "Today's Market"} - {formattedDate}
          </h2>
        </div>
        <div className="flex items-center gap-2 text-sm text-gray-500">
          <Calendar className="h-4 w-4" />
          <span>{language === 'bn' ? 'সর্বশেষ আপডেট' : 'Last Updated'}</span>
        </div>
      </div>
